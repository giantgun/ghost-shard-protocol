...
if (!privateKey || !rpcUrl) {
  throw new Error('Please set PRIVATE_KEY and RPC_URL in .env');
}
...
async function main() {
  ...
  const tx = await preparePrivateDeposit({
    metaAddress,
    amount: 3n * 10n ** 15n, // 1 ETH
    tokenAddress: account.address, // NATIVE
    type: 'NATIVE',
    senderAddress: account.address,
  });

  console.log("Sending to Meta Address:", metaAddress);
